...
router.post("/adminPanel", async (req, res, next) => {
    try {
        ...
        if (orderMethod) {
            if (filterObj) {
                filterObj.OrderMethod = orderMethod
        ...
        const pipeline = [filter_stage];
        // let orders = await PharmacyOrder.aggregate(pipeline).populate({ path: "userId" }).exec();
        let orders = await PharmacyOrder.aggregate(pipeline);
        let order1 = await PharmacyOrder.populate(orders, { path: "userId", select: "name" });
        let order2 = await PharmacyOrder.populate(orders, { path: "pharmacyId", select: "name" });
        ...
        res.status(200).json({ order1, order2, pickUpOrdersCount, deliveryOrdersCount });
    } catch (err) {
...
router.get("/getAll/:pharmacyId", async (req, res, next) => {
    try {
        ...
        let orders = await PharmacyOrder.find({ "pharmacyId": pharmacyId }).populate({ path: "userId", select: "phoneNo" })
...
router.delete("/delete/:id", isAuth, async (req, res, next) => {
    try {
        let orders = await PharmacyOrder.findByIdAndDelete(req.params.id);
