        ...
        socket.on("newOrder", async (data, cb) => {
            try {
                ...
                if (data.OrderMethod == "Pickup") {
                    ...
                    let pharmacys = await Pharmacy.find({ serviceOptions: 'pickup' })
                    ...
                    let pharmacys =await Pharmacy.find({ serviceOptions: 'delivery' })
                    pharmacys.forEach(element => {
    ...
    pharmacyNsp.on("connection", (socket) => {
        ...
        socket.on("orderCompleted", async (data, cb) => {
            try {
                ...
                let order = await PharmacyOrder.findByIdAndUpdate(data.id, { OrderStatus: "completed",platformFee:data.platformFee })

                let updateOrder=await PharmacyOrder.findById(data.id)
                console.log(updateOrder)
                // console.log(result);
                cb({ status: "success", data: updateOrder });
                // console.log(result, ";;;;;;;;;;;;;;");

                let user= await User.findById(order.userId);
                notify(user.fcmToken, "Order Completed", `pharmacy has completed your order`)
                userNsp.emit("orderCompleted", { status: "success", data: updateOrder });
