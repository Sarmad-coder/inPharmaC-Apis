...
const PharmacyOrder = require("../infrastructure/models/pharmacyOrder");
...
router.post("/register/withPhone", async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            return res.status(200).json({ code: 200, token: token, data: user, status: "success", message: "user login successfully" });
        }
...
router.post("/update", isAuth, async (req, res, next) => {
    try {
        ...
        res.status(200).json({ status: "success", data: user, message: "user updated" });
    } catch (error) {
        resolveError(error, res);
    }
});
...
router.get("/getById/:userId", async (req, res, next) => {
    try {
        let userId = new ObjectId(req.params.userId)
        let orders = await PharmacyOrder.find({ "userId": userId })
