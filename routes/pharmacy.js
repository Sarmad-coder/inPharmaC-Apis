...
router.put("/adminPanel/setBlock/:id", isAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const { block } = req.body;
        console.log(block, typeof block, id);
        await Pharmacy.updateOne({ _id: id }, { $set: { blocked: !block } })
...
router.post(
    ...
    async (req, res, next) => {
        try {
            let pharmacy = await Pharmacy.findById(req.body.id);
